...
class Products:
    ...
    def basket(all_products):
        ...
        if not all_products:
            # Print se o carrinho estiver vazio
...
def logic(service):
    ...
    while buy not in ['S', 's']:
        ...
        # VALIDAÇÃO para escolha do produto
        while True:
            try:
        ...
        # VALIDAÇÃO para quantidade desejada
        while True:
            try:
...
def user_score():
    ...
    stars_count = int(user_stars) * '*'

    user_name = input("\nDigite o seu nome: ")
    
    while user_name == '' or False == user_name.isalpha() :
        user_name = input("\nPor favor. Digite o seu nome: ")
    
    user_sugestion = input(
        "\nNos dê uma sugestão de como melhorar sua experiência! ")
    score = [stars_count, user_name, user_sugestion]

    print(color.BOLD + f"\nAgradecemos a sua avaliação, {user_name}!" + color.END)

    print(color.YELLOW + f"{stars_count}" + color.END)
...
while access_control in ['A', 'a']:
    ...
    if access_control in ['A', 'a']:
        ...
        Você ainda não realizou uma compra''' + color.END)
                ...
                if all_products:
                    print(color.RED + "ATENÇÃO! Pague suas compras antes de sair! " + color.END)
                    Products.basket(all_products)
                else:
                    access_control = 'a'
                    break
...
with open("vendas.csv", 'a') as sales:
